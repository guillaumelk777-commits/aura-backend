...
const app = express();
app.use(cors());
app.use(express.json({ limit: '50mb' })); // Permitir videos de hasta 50MB
...
// Obtener detalle de una batalla
...
// Crear batalla con límite de 5 activas por usuario
app.post('/api/battles', async (req, res) => {
  ...
  try {
    ...
    const q = await pool.query(
      ...
      [creator_id, category, title, media_url_creator || null]
    );
    res.status(201).json(q.rows[0]);
  } catch (e) { res.status(500).json({ error: e.message }); }
});

// Unirse como oponente
...
// Sistema de Votación y Motor de Resolución (+20 / -5 Aura)
app.post('/api/battles/:id/vote', async (req, res) => {
  ...
  try {
    ...
    // Regla de resolución: 10+ votos y diferencia de 3+
...
// Ranking de usuarios por Aura
app.get('/api/users/ranking', async (req, res) => {
  try {
    const q = await pool.query('SELECT * FROM users ORDER BY aura_points DESC NULLS LAST LIMIT 50');
    res.json(q.rows);
  } catch (e) { res.status(500).json({ error: e.message }); }
});
